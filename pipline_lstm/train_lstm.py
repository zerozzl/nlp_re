...
from pipline_lstm.model import LSTM
...
def get_dataset(args, rela_to_id, tokenizer, bigram_tokenizer):
    train_dataset = ReDataset(args.task, '%s/%s/train.txt' % (args.data_path, args.task), args.token_type, rela_to_id,
                              ...
                              do_to_id=True, add_entity_mark=True, do_sort=True, debug=args.debug)
...
def data_collate_fn(data):
    ...
    return relations, context, context_bigram
...
def train(args, dataset, dataloader, model, optimizer, lr_scheduler):
    ...
    for batch, data in enumerate(dataloader):
        ...
        loss = model(context, bigram=bigram, decode=False, tags=relations)
        loss = loss.mean()
...
def main(args):
    ...
    token_to_id, pretrain_embed = load_pretrain_embedding(args.pretrained_emb_path,
                                                          ...
                                                          add_pad=True, add_unk=True, add_entity_mark=True,
                                                          debug=args.debug)
    ...
    if args.pretrained_model_path is not None:
        ...
        model = LSTM(len(rela_to_id), len(token_to_id), args.embed_size, args.hidden_size,
                     args.input_dropout_rate, args.hidden_dropout_rate,
                     embed_fix=True if args.embed_type == 'static' else False,
                     use_bigram=args.use_bigram, bigram_vocab_size=len(bigram_to_id), bigram_embed_size=args.embed_size)
